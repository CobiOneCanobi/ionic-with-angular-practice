angular.module('thunder.controllers', ['ionic', 'thunder.services'])
  ...
  */
  .controller('DiscoverCtrl', function($scope, $timeout, User){
   $scope.products = [
    ...
    //fired when we favourite or skip a product
    $scope.sendFeedback = function(bool) {
      //first, add to favourites if favourited
      if (bool) { User.addProductToFavourites($scope.currentProduct); }

      //set the variable for the correct animation sequence
      $scope.currentProduct.rated = bool;
      $scope.currentProduct.hide = true;

      //timeout to allow animation to complete before changing to next product
      $timeout(function(){
  ...
  */
  .controller('FavouritesCtrl', function($scope, User){
    //get the list of our favourites from our user service
    $scope.favourites = User.favourites;

    $scope.removeProduct = function(product, index) {
      User.removeProductFromFavourites(product, index);
    };
  })
